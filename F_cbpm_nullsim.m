function D_cbpm_nullsim(outcome, connections, connectome, threshold, partialCor, i)
% ------------ INPUTS -------------------
...
% We need to create a custom job storage location to avoid a race

pc = parcluster('local');
[~, curSLURMJobId] = system('echo "$SLURM_JOB_ID"');
CustomjobLoc = fullfile(pc.JobStorageLocation, curSLURMJobId);
if ~exist(CustomjobLoc, 'dir')
    mkdir(CustomjobLoc);
end
pc.JobStorageLocation = CustomjobLoc;
mypool = parpool(pc, 12, 'SpmdEnabled', false);
...
switch partialCor
    case 'none'
        ...
        % 
        [behav_pred] = run_cbpm(all_mats, random_behav, threshold, []);
    ...
    case 'age+sex+fd'
        ...
        behav_pred = run_cbpm(all_mats, all_behav, threshold, [PredictTbl.Age PredictTbl.Sex PredictTbl.fd]);

    case 'age+sex+fd+acer+fluidintel'

        behav_pred = run_cbpm(all_mats, all_behav, threshold, [PredictTbl.fd PredictTbl.Age PredictTbl.additional_acer]);
...
% save the results
fileDir  = '/mmfs1/data/kurkela/Desktop/CamCan/code/nullsims';
if ~exist(fileDir, 'dir')
    mkdir(fileDir)
end
FN = sprintf('analysis-cbpm_outcome-%s_connections-%s_connectome-%s_thresh-%.03f_partialCor-%s_nullsim-%04d.mat', outcome, connections, connectome, threshold, partialCor, i);
full_file_name = fullfile(fileDir, FN);
save(full_file_name, 'R')

rmdir(CustomjobLoc, 's')
